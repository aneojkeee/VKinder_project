...
with open('config_db.json', 'r') as user_file:
...
def add_offer(vk_user_id, vk_offer_id, first_name, last_name, sex, age, city, black_list=0):
    ...
    if vk_offer_id not in [offer[0] for offer in offer_find]:
        ...
        session.add(offer)
    user_offer_find = session.query(m.User_Offer.user_offer_id).\
...
def add_interest(interest, vk_user_id=0, vk_offer_id=0):
    ...
    user_find = session.query(m.Interest_Person.vk_user_id).\
        filter(m.Interest_Person.interest_id == interest_id_find).\
        filter(m.Interest_Person.vk_user_id == vk_user_id).all()
    offer_find = session.query(m.Interest_Person.vk_offer_id).\
        filter(m.Interest_Person.interest_id == interest_id_find).\
        filter(m.Interest_Person.vk_offer_id == vk_offer_id).all()
    if vk_user_id != 0 and vk_user_id not in [user[0] for user in user_find]:
        interest_person_add = m.Interest_Person(vk_user_id=vk_user_id, interest_id=interest_id_find)
        session.add(interest_person_add)
    if vk_offer_id != 0 and vk_offer_id not in [offer[0] for offer in offer_find]:
        interest_person_add = m.Interest_Person(vk_offer_id=vk_offer_id, interest_id=interest_id_find)
        session.add(interest_person_add)
    session.commit()
...
def get_offer(vk_user_id):
    ...
    for note in offer:
        ...
        offer_interests = session.query(m.Interest.interest). \
            join(m.Interest_Person, m.Interest_Person.interest_id == m.Interest.interest_id). \
            filter(m.Interest_Person.vk_offer_id == note[0]).all()
        interest_list = []
